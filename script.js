...
async function callGeminiModel(promptText, opts={}){
  const model = GEMINI_MODEL || 'models/text-bison-001'
  const key = GEMINI_API_KEY
  if (!key) throw new Error('GEMINI_API_KEY not configured')
  const url = `https://generativelanguage.googleapis.com/v1beta2/${model}:generateText?key=${encodeURIComponent(key)}`
  const body = Object.assign({ prompt: { text: promptText }, temperature: 0.2, maxOutputTokens: 800 }, opts)
  const headers = {'Content-Type':'application/json'}
  // Gemeni may support API key in query string; include Authorization as a bearer token as fallback
  try{ headers['Authorization'] = `Bearer ${key}` }catch(e){}
  const resp = await fetch(url, {method: 'POST', headers, body: JSON.stringify(body)})
  const data = await resp.json()
  // parse content from standard Generative Language API
  const content = data?.candidates?.[0]?.content || data?.output?.text || (Array.isArray(data?.candidates) && data.candidates[0] && data.candidates[0].content) || JSON.stringify(data)
  return content
}
...
async function loadFile(file){
  ...
  if (type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')){
    ...
    try{
      ...
      alert('Failed to parse PDF locally.'); console.error(err)
      return
    }
    ...
    alert('Unsupported file type. Please provide PDF or TXT files.')
  }
}
...
function computeATSSscore(analysis, txt){
  // Naive scoring combining detections
  let score = 50
  const breakdown = {base:50}
  if (analysis.contactInfo && analysis.contactInfo.email) { score += 10; breakdown.email = 10 }
  if (analysis.contactInfo && analysis.contactInfo.phone) { score += 10; breakdown.phone = 10 }
  if (analysis.sections && (analysis.sections['skills'] || (analysis.skills && analysis.skills.length>0))){ score += 10; breakdown.skills = 10 }
  if (analysis.sections && (analysis.sections['experience'] || txt.toLowerCase().includes('experience'))) { score += 10; breakdown.experience = 10 }
  if (txt && txt.length && txt.length < 800) { score -= 10; breakdown.length = -10 }
  // penalize missing metrics
  if (!txt.match(/\d+%|\d+ years|\d+\+|\d+ months/)) { score -= 10; breakdown.metrics = -10 }
  // score by number of skills and keywords
  const skillCount = (analysis.skills||[]).length
  if (skillCount >= 3){ score += Math.min(10, skillCount * 2); breakdown.skillCount = Math.min(10, skillCount * 2) }
  // add small weight for roles detected
  const roleCount = (analysis.roles||[]).length
  if (roleCount>0){ const add = Math.min(8, roleCount*3); score += add; breakdown.roles = add }
  // count number of sections for structure
  const sectionCount = Object.keys(analysis.sections||{}).length
  if (sectionCount >= 3) { score += 5; breakdown.sections = 5 }
  // reward measurable achievements presence (find multiple metrics)
  const metricsFound = (txt.match(/\d+%|\d+ years|\d+\+|\d+ months/g) || []).length
  if (metricsFound >= 1){ const addm = Math.min(10, metricsFound * 3); score += addm; breakdown.metricsCount = addm }
  // cap
  const finalScore = Math.min(100, Math.max(0, score))
  console.log('ATS breakdown:', breakdown, 'final:', finalScore)
...
async function analyzeAndRender(){
  if (!state.cleanedText) return alert('No resume loaded')
  console.log('analyzeAndRender: starting with cleanedText length', state.cleanedText?.length)
  ...
  if (format === 'txt'){
    ...
    if (window.docx){
      ...
      for (const l of paraLines){
        doc.addSection({children: [new docx.Paragraph({children: [new docx.TextRun(l)]})]})
      }
      docx.Packer.toBlob(doc).then(blob => saveBlob(blob, filename + '.docx'))
    } else {
      // fallback to .doc
      const html = `<html><head><meta charset='utf-8' /></head><body><pre style='font-family:Calibri;'>${escapeHtml(content)}</pre></body></html>`
      const blob = new Blob([html], {type:'application/msword'})
